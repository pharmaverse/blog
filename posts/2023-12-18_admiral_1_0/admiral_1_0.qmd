...
pkg <- function(package) {
  url <- downlit::href_package(package)
  glue::glue("<a href='{url}'>{package}</a>")
}

fun <- function(fun) {
  parts <- strsplit(fun, "::")[[1]]
  url <- downlit::href_topic(parts[2], package = parts[1])
  glue::glue("<a href='{url}'>{fun}()</a>")
}
```
...
* `r fun("admiral::derive_var_merged_ef_msrc")`
* `r fun("admiral::derive_vars_computed")`
