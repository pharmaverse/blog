...
author:
  - name: Liam Hobby
description: "{metacore}/{metatools} 0.2.0 are live. Read package release information and hear from the new maintainer."
date: "2024-12-01"
...
The goal of this update was to clarify the distinction between an inported Metacore spec, containing information about multiple datasets, and a subsetted spec containing information about just a single dataset (as achieved via `metacore::select_dataset`).
...
Now, a Metacore object about multiple datasets and one about a single dataset have been redesigned to be programmatically distinct, with the single dataset implemented as a subclass of Metacore called "DatasetMeta".

From the users perspective there is one key change.
**A metadata object about a single dataset will be required for users to work with {metatools} functions**, which have had their API harmonised to accept only subsetted Metacore objects (`metacore::select_dataset`).
...
The `select_dataset` function is now explicit about what is being selected:
...
Additionally, the argument `dataset_name` has been **soft-decprecated** across all functions in {metatools}.
...
`create_var_from_codelist` now optionally allows the user to specify a codelist from which the new column should be generated.
...
```{r}
adlbc <- suppressMessages(select_dataset(metacore, "ADLBC", quiet = TRUE))
data <- tibble(PARAMCD = c("ALB", "ALP", "ALT"))
create_var_from_codelist(data, adlbc, input_var = PARAMCD, out_var = PARAM, strict = FALSE)
```

By default, `create_var_from_codelist` takes the codelist of the `out_var` as input.
...
```{r}
create_var_from_codelist(data, adlbc, input_var = PARAMCD, out_var = PARAM, codelist = get_control_term(adlbc, PARAMCD), decode_to_code = FALSE)
```
...
x <- create_var_from_codelist(data, adlbc, input_var = PARAMCD, out_var = PARAM, codelist = get_control_term(adlbc, PARAMCD), decode_to_code = FALSE, strict = TRUE)
```

## `metatools::create_cat_var()`

`create_cat_var` has been updated so that users can now specify to create a new variable from either the `code` or `decode` column of the controlled terminology.
...
create_cat_var(dm, adsl, AGE, AGEGR2, AGEGR2N, create_from_decode = TRUE) %>%
...
x <- create_cat_var(dm2, adsl, AGE, AGEGR2, create_from_decode = TRUE)
```

## Summary of Other Changes

-   `metacore` Fixed a bug where the presence of variables with VLM in the `value_spec` table would prevent variables of the same name in different datasets being populated in the `value_spec` table.
