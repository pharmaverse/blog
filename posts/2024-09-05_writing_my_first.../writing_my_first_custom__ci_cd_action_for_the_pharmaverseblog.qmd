...
Users **can** add to this list, however we have observed that more often than not, if users do diverge from it is due to a typo. This has unfortunate effects within our pharmaverseblog, chiefly that our front page glossary of posts is now split:
...
![](cicd_pipeline.png){fig-align="center" width="500"}

:::

The pharmaverseblog already had three active CICD pipelines, which run for spelling, style and links. We did not write the code for these checks ourselves, rather we just activated open-source checks for our code repository. But what if we could write our own, **custom CICD pipeline** to check that new blog posts use tags from our selected list?
...
* I wrote a function, `check_post_tags()` which scans the `.qmd` file containing a blog post, extracts the tag(s), and checks it against the allowed list.
* I then set up a script which loops `check_post_tags()` over all our blog posts and identifies offending posts.
* Then, I used the `{cli}` package to format a nice error message for all the offending posts and their tags.
* In `.github/workflows`, I created a new CICD workflow named `check_post_tags.yml` which simply executes my script upon every pull request to `main`.
