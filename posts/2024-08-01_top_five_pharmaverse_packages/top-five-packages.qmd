...
Next let’s take a look at the [**{admiral}**](https://pharmaverse.github.io/admiral/) package, developed through a collaboration between [Roche](https://www.roche.com/) and [GSK](https://www.gsk.com/en-gb/).
It provides a toolbox of reusable functions and utilities with [**dplyr**](https://dplyr.tidyverse.org/)**-like syntax** to prepare **CDISC ADaM (Analysis Data Model)** **datasets**.
It serves as a valuable resource for statistical programmers looking to build ADaMs according to varying analysis needs while ensuring traceability and compliance with **FDA standards**.
...
-   **Modular design allows for extensions** that focus on specific areas or needs, 3 public packages are currently extending {admiral}, such as [{admiralonco}](https://github.com/pharmaverse/admiralonco), [{admiraloptha}](https://github.com/pharmaverse/admiralophtha) and [{admiralvaccine}](https://github.com/pharmaverse/admiralvaccine).
    All of them were created from the [admiral template](https://github.com/pharmaverse/admiraltemplate).
...
[**{teal}**](https://github.com/insightsengineering/teal)is an open-source R Shiny framework developed by [Roche](https://www.roche.com/) that enables the creation of interactive data exploration applications for the pharmaceutical industry.
[{teal}](https://insightsengineering.github.io/teal/latest-tag/) is particularly well-suited for exploring and analyzing data from clinical trials, enabling researchers and clinicians to quickly identify trends, patterns, and insights.\
...
-   Its **reporter functionality** allows users to **build custom reports** based on the visualizations filtered and parametrized in the app.
    Users can generate PDFs or PPT documents that include images and optional code.
...
The [**{riskmetric}**](https://pharmar.github.io/riskmetric/articles/riskmetric.html) package provides a framework to quantify the "risk" of R packages by assessing various metrics.
Developed by the [**R Validation Hub**](https://www.pharmar.org/), it helps organizations evaluate the quality and suitability of R packages for validated environments.
The resulting risk is parametrized by the organization, which makes the decision on how to weigh the risk from the different metrics.
...
[**{tidyCDISC}**](https://github.com/Biogen-Inc/tidyCDISC/)is an open-source R package developed by [Biogen](https://www.biogen.com/) that provides a set of functions for tidying and manipulating [CDISC](https://www.cdisc.org/) (Clinical Data Interchange Standards Consortium) datasets.
It aims to simplify the process of working with [CDISC](https://www.cdisc.org/) data by providing an intuitive interface for data transformation tasks and ensuring consistency with the principles of tidy data.
