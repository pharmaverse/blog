...
This blog post will showcase some of the highlights of this release, but you can explore the full contents in our [Changelog](https://pharmaverse.github.io/admiral/cran-release/news/index.html).
...
`{admiral}` now offers a new metadata object, country_code_lookup(), that can aid metadata added to decode countries based on [ISO 3166 codes](https://www.iso.org/iso-3166-country-codes.html).
Our hugely-flexible derive_vars_merged() then comes in handy if you ever need to leverage this metadata:
...
Driven by a request from our GSK users, the `{admiral}` back-end now contains helper functions to derive_vars_merged() (get_flagged_records()) and derive_vars_query() (get_vars_query()) so that those can be called independently if required.
...
The function derive_var_trtemfl() now comes with a new (optional) argument, `group_var`.
...
In this release cycle, we took the opportunity to review and revamp our [Get Started](https://pharmaverse.github.io/admiral/cran-release/articles/admiral.html) vignette.
...
Coupled with the rework to the [Get Started](https://pharmaverse.github.io/admiral/cran-release/articles/admiral.html) vignette, we now have a brand-new [Programming Concepts and Conventions](https://pharmaverse.github.io/admiral/cran-release/articles/concepts_conventions.html) User Guide.
...
* New release of {admiralonco} (1.1) featuring a new vignette for creating ADRS with IMWG criteria ([full changelog here](https://pharmaverse.github.io/admiralonco/cran-release/news/index.html)).
