...
While some trials fail to either inform participants of potential risks or give an accurate representation of diverse populations, many trials do not report negative or inconclusive results. Industry sponsored trials may also introduce bias in any one of the design, analysis, or reporting stages.

### Cost and Efficiency Challenges

Clinical trials are expensive, often exceeding hundreds of millions of dollars, and this can limit the development of drugs for rare diseases. Failure in any phase of these trials can incur huge losses, compounded by the complex and varying nature of global regulations around them.

### Data and Reproducibility Concerns

Reproducibility is key for independent verification of clinical trial results. Differences in data collection methods, formats and methodologies often hinder cross-study comparisons, especially if a study was not designed with international standards that are consistent, and reproducible. If you'd like to hear some direct industry insights on Clinical Trials, check out this podcast on [Why Clinical Trials Fail: Lessons from Two Decades in Pharma](https://www.youtube.com/watch?v=UXfUUA1vo4I&list=PLexAKolMzPcpRNtpwdAwRO9fCxAdwDauo)
...
SDTM is a standard for ***structuring and building datasets*** for individual clinical study data, ensuring a consistent format for regulatory submissions (e.g. to the FDA, EMA). In other words, this model defines a ***standardized format for collecting and organizing raw clinical trial data*** which allows for easier cross-study comparisons and improves data transparency.

### ADaM - Analysis Data Model

ADaM ensures traceability from SDTM data to analysis-ready datasets. This model establishes a ***standardized way to create datasets for statistical analysis*** from SDTM organized data, facilitating efficient analysis and reporting.

## The Role of R in Clinical Trials

R is widely used in clinical trial analysis due to its flexibility, powerful statistical capabilities, and strong support for regulatory compliance. R provides:
...
Pharmaverse is a collaborative open-source ecosystem of R packages tailored for clinical trial data processing and regulatory compliance. It provides tools that simplify the implementation of CDISC standards in R.

-   Key Pharmaverse Packages:
    -   **`sdtm.oak`** – Helps in structuring SDTM datasets.
    -   **`admiral`** – Automates ADaM dataset creation.
...
Can’t wait to get your hands on some Pharmaverse tools? We got you covered! [https://www.appsilon.com/post/pharmaverse-tools-for-clinical-trials](https://hubs.ly/Q03m5bG30){.uri}

## Data Pipeline

In Clinical Trial Reporting, the data pipeline follows this standard:
...
In this example, we will be creating a ***Standard Adverse Events Table (AET02)*** using the c`adsl` and c`adae` dataset from the `random.cdisc.data` package.
...
In this example, we will be creating a ***Standard Listing of Preferred Terms, Lowest Level Terms, and Investigator-Specified Adverse Event Terms*** ***(AEL01)*** using the c`adae` dataset from the `random.cdisc.data` package.
...
More specifics on these outputs can be found in the [TLG-Catalog](https://insightsengineering.github.io/tlg-catalog/stable/%20).

Did you know that validating R Shiny applications is critical in the pharmaceutical industry? Want to know more? [Read on in this blog post.](https://hubs.li/Q03m5zwM0)\
